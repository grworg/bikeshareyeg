...
if [ "$SYNC_DATA" = true ]; then
  ...
  # Precomputed hex grid (generated by scripts/precompute-hexgrid.py)
  if [ -f data/hexgrid.geojson ]; then
    rsync -az data/hexgrid.geojson "$SSH_HOST:$REMOTE_DIR/data/hexgrid.geojson"
    ok "Hex grid synced ($(du -h data/hexgrid.geojson | cut -f1))"
  else
    err "data/hexgrid.geojson not found — run: make hexgrid"
  fi
